...
load(paste0(workdir, "Fig. 10b_source_data1_Kidney_ccRCC_umap.Rdata"))
...
ggsave(path = workdir, filename = "Fig. 10b.png", width = 10, height = 6, device='png', dpi=300)
...
ggplot(temp_umap, aes(x=umap_1, y=umap_2, color=label)) + 
  ...
  theme(text=element_text(size=15,  family="serif",face = 'bold'),
        ...
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  guides(color = guide_legend(ncol =1, override.aes = list(size = 4)))
ggsave(path = workdir, filename = "Fig. 10b_Rlab.png",width = 10, height = 6, device='png', dpi=300)
...
ggplot(temp_umap, aes(x=umap_1, y=umap_2, color=label)) + 
  ...
  theme(text=element_text(size=15,  family="serif",face = 'bold'),
        ...
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  guides(color = guide_legend(ncol =1, override.aes = list(size = 4)))
ggsave(path = workdir, filename = "Fig. 10b_scCAD.png",width = 10, height = 6, device='png', dpi=300)
